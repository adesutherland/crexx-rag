...
model/dimension at a time and can be rebuilt from SQLite metadata. The CLI can
batch-embed stored chunks through an external command, and MCP has the same
external-command hook for query-time vectors. Richer provider integration and
ranking policy still belong above the core.
