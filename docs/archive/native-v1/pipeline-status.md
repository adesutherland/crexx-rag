...
> [!CAUTION]
> Archived and frozen native-v1 status snapshot. The living programme status is
> [docs/pipeline-status.md](../../pipeline-status.md).
...
The cREXX-only replacement is a proposed, gated programme, not current behavior.
Its [specification](../../crexx-only-vision-and-specification.md),
[architecture](../../crexx-only-architecture.md), and
[implementation roadmap](../../crexx-only-implementation-roadmap.md) preserve this
pipeline as the executable oracle until a separately approved cutover. Generic
CREXX facilities may be incubated here and donated upstream, but none of those
documents changes the implemented status below.
