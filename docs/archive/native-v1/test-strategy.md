...
> [!CAUTION]
> Archived native-v1 test strategy. Use the living
> [cREXX-only test strategy](../../test-strategy.md) for programme acceptance.
...
The cREXX-only migration has an additional, resumable acceptance matrix in
[`crexx-only-implementation-roadmap.md`](../../crexx-only-implementation-roadmap.md).
It adds oracle fixtures, true source-revision lifecycle, normalized claim
support, lease/crash tests, provider/privacy parity, retrieval judgements,
installed-package tests, and same-session cREXX performance evidence. Those are
cutover gates; the current tests below remain the shipped-path baseline.
