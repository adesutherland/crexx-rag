...
> [!CAUTION]
> Archived former repository instructions. They apply only when reproducing or
> maintaining the native-v1 oracle. Current agents must follow the root
> [AGENTS.md](../../../AGENTS.md).
...
## cREXX-Only Migration Programme

The programme target is a cREXX application rather than a RAG-specific native
core. Read these programme documents before migration work:
...
The current native implementation remains the executable correctness oracle
until the roadmap's explicit cutover gate. Existing native design and build
rules below therefore remain authoritative for current code.

For migration work:

- Put product/domain algorithms, SQL repositories and migrations,
  orchestration, policy, configuration, CLI behavior, jobs, retrieval, and
  evidence assembly in cREXX.
- Native capabilities may be incubated in this repository before donation to
  CREXX, but their APIs, tests, examples, and packaging must be independently
  useful and contain no RAG concepts.
- Treat this project as a non-trivial CREXX reference application. Record
  minimized capability failures, same-session workload measurements, and
  donation-ready evidence instead of hiding surface weaknesses in
  product-specific native helpers.
- For architecture planning, assume the sister CREXX PERF2 programme completes
  successfully. Do not edit, sequence, or interfere with its ongoing work;
  cREXX-rag still measures its own storage, data-transfer, provider, graph, and
  vector workloads before selecting application boundaries.
- Do not translate the current fixed-buffer JSON bridge into the new design.
  Prefer typed SQLite cursors/values and cREXX records; keep JSON at external
  boundaries.
- Do not dual-write a live library. Run the current and target paths against
  copies/fixtures and compare semantic results.
- Do not remove the C++ core, stable C ABI, `rx_rag` bridge, current bundle
  format, or compatibility commands without a separate accepted cutover
  decision.
- Keep every target-interface document explicit about proposed versus shipped
  behavior until the corresponding roadmap gate is accepted.
...
The repeatable pattern is archived at
`docs/archive/native-v1/crexx-plugin-pattern.md`.
...
- Keep local embedding providers optional and adapter-level. The first open
  ...
  `docs/archive/native-v1/local-embeddings.md`; Ollama remains supported through the same
...
- The first measured Scotland Stage 3 batch processed 100 queued chunks in about
  ...
  runs extended the processed total; see
  `docs/archive/native-v1/pipeline-status.md` for the frozen milestone status.
  Treat Stage 3 and later improvement passes as background
...
- Native-v1 oracle reproduction should use
  `docs/archive/native-v1/tutorial-import-improve-query.md` for the
  ...
  extraction.
- The former living pages are frozen at
  `docs/archive/native-v1/pipeline-status.md` and
  `docs/archive/native-v1/test-strategy.md`. Update the current programme status
  pages rather than treating these archived pages as mutable guidance.
- Keep the cREXX-only specification, architecture, roadmap, user guide, and
  generic agent instructions mutually consistent whenever migration behavior or
  a programme gate changes.
