...
> [!CAUTION]
> Archived native-v1 oracle documentation. This file preserves the former
> shipped build and command surface; it is not current product direction. Start
> with the [current documentation map](../../README.md).
...
## cREXX-Only Target Programme

The next architecture programme returns the project to its original goal: the
product algorithms, storage repositories, orchestration, policy, configuration,
commands, jobs, retrieval, and agent-facing behavior should be implemented in
cREXX. Native code should remain only behind reusable CREXX facilities such as
SQLite and, if real measurements require it, vector acceleration.

This is also a deliberate CREXX proving programme. Generic capabilities may be
incubated and tested in this repository, exercised by the real RAG/graph
workload, and then donated to the sister CREXX project with independent APIs,
tests, documentation, packaging, and benchmarks.

The current C++ core remains the working implementation and executable oracle;
the target is proposed, not yet the default. The programme documents are:

- [review findings](../../crexx-only-review-findings.md);
- [vision and product specification](../../crexx-only-vision-and-specification.md);
- [target architecture](../../crexx-only-architecture.md);
- [resumable implementation roadmap](../../crexx-only-implementation-roadmap.md);
- [target user guide](../../crexx-only-user-guide.md); and
- [generic LLM agent instructions](../../../prompts/crexx-rag-agent-AGENTS.md).

The roadmap requires measured cREXX capability slices and side-by-side semantic
parity before broad translation. Removing the native core or changing the
default bundle format requires a separate cutover decision.

## Current Shipped Implementation

The remainder of this README describes the current native-core proof of concept
and its executable commands. It remains authoritative until the corresponding
cREXX-only roadmap gates are accepted.
...
[`how-it-fits-together.md`](how-it-fits-together.md). It covers
...
documented in [`local-embeddings.md`](local-embeddings.md), using the
...
[`docs/crexx-integration-issues.md`](../../crexx-integration-issues.md).
...
[`docs/architecture-vocabulary.md`](../../architecture-vocabulary.md).
...
runs extended that total; see `docs/archive/native-v1/pipeline-status.md` for the frozen milestone
...
See archived [`pipeline-status.md`](pipeline-status.md) for implemented versus
planned native-v1 status, and archived [`test-strategy.md`](test-strategy.md)
for acceptance criteria. Executable operator/agent workflows are in
[`use-cases.md`](use-cases.md). A full first-run tutorial is in
[`tutorial-import-improve-query.md`](tutorial-import-improve-query.md).
...
The cREXX-only documents linked above supersede that final sentence as the
long-term direction. Until their migration gates are accepted, this section and
the current CLI/tutorial continue to describe shipped behavior.
