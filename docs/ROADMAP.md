...
| ESC-OPS-03 | Open — isolated reproduction required | One validated type-correction failed at `ragbacklog` maintenance_items INSERT with `cannot record validated lifecycle action`. Original response/usage retained, no decision, clean final verification. The generic message hides the SQLite reason; a key/uniqueness collision is a hypothesis, not diagnosis or proof of contention. [Run, exact task/item and next bounded acceptance](maintenance-budget-soak-20260917.md). |
