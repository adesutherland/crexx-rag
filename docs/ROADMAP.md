...
| ESC-OPS-02 | Repaired and locally qualified; installation pending | The [15-minute reference soak](resolution-references-soak-20260917.md) found an ordinary escalation → advanced search → read sequence whose read prompt advertised two calls remaining, but the task then failed at three total attempts without a concluding call. The [isolated reproduction and repair](advanced-call-budget-delivery-20260917.md) identifies a shadowed task reference in the shared route counter. A single SQL alias repair keeps selection/exhaustion aligned with inspection and prompts. Three/five-call worker sequences pass on both VMs, including old failed-state reconsideration and total call/token accounting. All 126 required local cases pass, including shared admission, correction budgets, retained receipts, reset and installed CLI/MCP acceptance. No limit increase or corpus reset; Scottish installation/live repeat remain pending. |
...
acceptance and all 126 local checks. Installation/live repeat of this final
repair remain pending. Keep Luna Low / Sol Medium.
