...
lanes={'followups': ('/private/tmp/crexxrag-smoke-repeat-20260914-718axbnf/artifact/bin/crexxrag', '/private/tmp/crexxrag-smoke-repeat-20260914-718axbnf/library', '/private/tmp/crexxrag-smoke-repeat-20260914-718axbnf/test2.conf')}
...
try:
 rpc("initialize","initialize",{"protocolVersion":"2025-06-18","capabilities":{},"clientInfo":{"name":"crexxrag-route-followup","version":"1"}})
 rpc("sources","tools/call",{"name":"rag_source_list","arguments":{"limit":50}})
 prior=json.loads((root/"repeat/mackay-h3-repeat.json").read_text())
 ev=json.loads(prior["result"]["structuredContent"]["records"][0]["fields"]["evidence_json"])
 endpoints=ev["leads"][0]["path"][:2]
 question="What is the relationship between "+endpoints[0]+" and "+endpoints[1]+"?"
 rpc("follow-returned-lead","tools/call",{"name":"rag_query_inspect","arguments":{"question":question,"hops":3,"direction":"both","limit":12}})
 data=rpc("bannockburn-full-evidence","tools/call",{"name":"rag_query_inspect","arguments":{"question":"What happened at Bannockburn in 1314?","hops":3,"limit":12}})
 ev=json.loads(data["records"][0]["fields"]["evidence_json"])
 passage=next(p for p in ev["passages"] if p["title"]=="bannockburn-1911-excerpt.txt" and "defeated Edward" in p["text"])
 rpc("bannockburn-battle-citation","tools/call",{"name":"rag_citation_show","arguments":{"citation":passage["citation"]["id"]}})
finally:
 proc.stdin.close()
