# Roadmap snapshot before reconciliation — 16 September 2026

Historical evidence only. [ROADMAP.md](ROADMAP.md) is the sole current-status
register. The snapshot below preserves the former wording and dated results;
its latest/current/open labels and execution instructions are not current.
...
| CREXX #701 — child pipe inheritance | Upstream open; local investigation closed, 15 September | Standard launcher descriptor cleanup belongs in CREXX. Accept delayed worker completion/replacement pending the upstream repair; no extra RAG monitoring or local runtime fix. Adrian approved temporarily disabling `worker_unexpected_exit`; retain it as not run and re-enable after the runtime fix. [Issue](https://github.com/adesutherland/CREXX/issues/701), [integration record](integration-issues.md#child-pipe-inheritance--transferred-upstream-local-investigation-closed). |
