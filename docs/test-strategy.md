...
| `worker_recovery` | native managed-process disconnects; bounded replacement and durable restart ceiling; no preflight phantom calls; public exact-turn inspection, stale digest and ambiguous-history holds, atomic reconciliation rollback, repeated settlement, completed-output reuse and bounded retry; OCR/multiple-citation feedback, uncalled correction deferral, advanced-reasoning handoff; admission-release diagnostics |
