...
| CRI-16 | Upstream substrate qualified; downstream lifecycle decision open | Current installed Level-G `rxfnsg` supplies typed responses, connection-owner pooling/keep-alive, bounded request/response handling, gzip/deflate, streaming and cancellation primitives, with supported-platform feature evidence and Linux sanitizer qualification in the pulled lineage. The macOS downstream providers compile and pass through that surface. Provider adapters intentionally still report cross-operation reuse, streaming and cancellation as unsupported because they own one pool per operation. Closure requires exact downstream Linux/package replay plus an approved provider-lifecycle and capability scope; it does not require pretending substrate support is already an adapter feature. |
| CRI-17 | CREXX infrastructure: native RXPA provider discovery inside attached tasks open | The SQLite provider itself now uses RXPA V2 per-VM sessions and `SQLITE_OPEN_FULLMUTEX`; a four-thread provider-session lifecycle harness passes, and the installed dynamic cREXX path reports session entry. Separate worker processes can load the provider and open independent connections normally. The installed attached-task path cannot seal a controller generation after a native module is loaded (`RXVM_PROGRAM_NATIVE_EXCLUDED`), while sealing first and autoloading later leaves the child-thread task VM without a provider search path. Closure requires CREXX to support declarative provider loading in each attached task VM with its normal RXPA session lifecycle. This is not a SQLite library or plugin defect and does not block process workers; until closure, an attached task transfers ordinary inputs/results and its controller owns SQLite. |
