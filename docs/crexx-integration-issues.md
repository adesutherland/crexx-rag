...
has opened CRI-15 for the installed socket receive timeout contract. This does
not authorize Phase 1B, production hardening, donation, a CREXX edit, cutover,
or native-v1 retirement.

## CRI-01 through CRI-15 current disposition
...
CRI-15 is the one open installed-CREXX compatibility item found by Linux
qualification. It blocks a Linux claim for deterministic structured provider
receive timeout behavior; it does not invalidate the selected provider-neutral
boundary or the retained Apple replay. The other still-bounded items are
product decisions: Phase-1B boundary approval, production schema and migrations,
provider hardening, representative vector crossover measurement,
