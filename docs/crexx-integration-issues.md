...
Status date: 2026-08-23. The current installed-only macOS replay uses CREXX
...
downstream inventory passes on macOS. Exact downstream Linux confirmation and
any provider-lifecycle expansion remain open. Neither issue authorizes a
...
design decision. The macOS SHA-256 and RXVECTOR capability work is complete.
