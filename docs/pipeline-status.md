...
| Endpoint/ambiguity resolution | Partial | Backlog can be represented in `work_queue`; a consumer is still needed. |
...
- An endpoint-resolution backlog exists and needs a consumer.
...
- Dry-runs record `dry-run`, not `skipped`, so test probes do not poison future
  ranking.
