...
The maintained
[Phase-8 opportunity report](../docs/reports/phase-8-donation-opportunities.md)
assesses donation priority, already installed adoptions, compatibility, and
retirement readiness. It identifies SQLite as the nearest opportunity while
keeping every bundle unsubmitted and every approval boundary unchanged.

| Capability | Implementation boundary | Colocated documentation | Disposition |
| --- | --- | --- | --- |
| SQLite (`rxsqlite` candidate) | [`p1a/sqlite_boundary/`](p1a/sqlite_boundary/) | [use](p1a/sqlite_boundary/README.md), [system](p1a/sqlite_boundary/SYSTEM.md), [package](p1a/sqlite_boundary/PACKAGE.toml), [bundle](p1a/sqlite_boundary/BUNDLE.tsv), [retained contract](phase1b/rxsqlite/CONTRACT.md) | Implemented generic dynamic/static provider pair using the typed RXPA surface and V2 per-VM sessions; four concurrent sessions, serialized SQLite connections and isolated diagnostics pass on macOS. Final naming, independent release packaging, exact Linux/platform QA, the CREXX task/native-provider seam, and donation approval remain open |
| Provider-neutral LLM/embedding (`rxllm` candidate) | [`phase1b/provider/`](phase1b/provider/) | [use](phase1b/provider/README.md), [system](phase1b/provider/SYSTEM.md), [Codex use](phase1b/provider/README-CODEX.md), [Codex system](phase1b/provider/SYSTEM-CODEX.md), [package](phase1b/provider/PACKAGE.toml), [bundle](phase1b/provider/BUNDLE.tsv) | Implemented cREXX incubation with Gemini/OpenAI-compatible HTTP routes and an experimental contained Codex App Server structured-generation route. Phase 3 exercises local `llama.cpp` embeddings and subscription-aware Codex durability; provider-lifetime scope, installed package proof, selected downstream Linux replay, catalogue policy and donation approval remain open |
...
| Industrial HTTP/TLS | Installed Level-G `rxfnsg`, `rxsocket`, and `_rxhttpcore`; local consumer in [`phase1b/provider/provider_http.crexx`](phase1b/provider/provider_http.crexx) | Provider [system documentation](phase1b/provider/SYSTEM.md) records the dependency and remaining downstream gates | The product uses bounded synchronous connection-per-attempt HTTP/TLS and completed a two-call Gemini slice. Exact downstream Linux confirmation and provider-lifetime reuse/stream/cancel decisions remain open |
