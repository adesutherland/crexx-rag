...
execute_process(COMMAND "${CPRAG_RXC}" -i "${imports}"
    -o "${CPRAG_WORK_DIR}/native_vector" "${CPRAG_SCENARIO}"
...
execute_process(COMMAND "${CPRAG_RXAS}" -o "${CPRAG_WORK_DIR}/native_vector"
    "${CPRAG_WORK_DIR}/native_vector"
...
foreach(runtime_name IN ITEMS rxvme rxbvm)
    ...
    execute_process(COMMAND "${runtime}"
        ...
        -l "${imports}" "${CPRAG_WORK_DIR}/native_vector" ${modules}
        -a "${library}"
...
message(STATUS "Native vector publication, retrieval, visibility and recovery passed on both VMs")
