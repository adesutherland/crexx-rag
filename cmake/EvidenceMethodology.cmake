...
set(modules ragenrich ragproposalio ragperiod ragprovenance ragassessment ragmodel ragevidence
    ...
    ragfolder ragclaimrules ragclaims ragimprove ragmaintain ragbacklog ragadmission raglifecycle ragworktypes ragenvironment ragusage ragreceipts ragwork ragquery ragembedding
...
execute_process(COMMAND "${CPRAG_RXC}" -i "${imports}"
    -o "${CPRAG_WORK_DIR}/evidence_methodology" "${CPRAG_SCENARIO}"
    RESULT_VARIABLE compile_result OUTPUT_VARIABLE compile_out ERROR_VARIABLE compile_err)
if(NOT compile_result EQUAL 0)
    message(FATAL_ERROR "Evidence methodology scenario compile failed:\n${compile_out}${compile_err}")
endif()
execute_process(COMMAND "${CPRAG_RXAS}" -o "${CPRAG_WORK_DIR}/evidence_methodology"
    "${CPRAG_WORK_DIR}/evidence_methodology"
    RESULT_VARIABLE assemble_result OUTPUT_VARIABLE assemble_out ERROR_VARIABLE assemble_err)
if(NOT assemble_result EQUAL 0)
    message(FATAL_ERROR "Evidence methodology scenario assembly failed:\n${assemble_out}${assemble_err}")
endif()
...
foreach(runtime_name IN ITEMS rxvme rxbvm)
    ...
    execute_process(COMMAND "${runtime}"
        ...
        -l "${imports}" "${CPRAG_WORK_DIR}/evidence_methodology" ${modules}
        -a "${library}"
