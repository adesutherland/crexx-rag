foreach(required_var CPRAG_RXC CPRAG_RXAS CPRAG_RXVME CPRAG_RXBVM
        CPRAG_CREXX_BIN_DIR CPRAG_MODEL CPRAG_TRACE CPRAG_COMMAND CPRAG_SOURCE CPRAG_WORK_DIR)
...
file(SHA256 "${CPRAG_TRACE}" trace_before)
...
foreach(mode IN ITEMS noopt opt)
    ...
    compile_crexx("${CPRAG_TRACE}" "${CPRAG_WORK_DIR}/ragtrace"
    ...
    foreach(runtime_name IN ITEMS rxvme rxbvm)
        ...
        execute_process(COMMAND "${runtime}" -l "${program_import}"
            "${CPRAG_WORK_DIR}/program-${mode}" ragmodel ragtrace ragcommand library
            -a "${cell}"
        ...
        if(NOT vm_result EQUAL 0 OR NOT vm_out MATCHES
                "P2_07_COMMAND_OK cell=${cell} operations=44 exit_codes=11 formats=human,json,ndjson json_schema=crexx-rag.command-result/1 parser_writes=0 provider_calls=0")
...
message(STATUS
    "P2-07 maintained contract passed: 44-operation argv parser, 11 stable exits, bounded versioned human/JSON/NDJSON rendering, negative syntax/result cases, and zero-write source fingerprints across noopt/opt x rxvme/rxbvm")
