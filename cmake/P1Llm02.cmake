...
find_program(local_llama_server NAMES llama-server)
if(local_llama_server)
    set(local_status "available executable=${local_llama_server};not_run=no_model_authorized")
else()
    set(local_status "unavailable reason=llama-server-executable-not-found")
endif()
file(WRITE "${report}"
    "contract=${CPRAG_CONTRACT}\nadapter=${CPRAG_ADAPTER}\nsource=${CPRAG_SOURCE}\n"
    "local_llama_server=${local_status}\nhosted_calls=0\n")
...
execute_process(COMMAND /bin/sh -c
    "( \"$1\" \"$2\" 8; printf '%s' $? >\"$5\" ) >\"$3\" 2>\"$4\" &"
    p1-llm-02 "${CPRAG_LOOPBACK}" "${port}" "${server_out}"
    "${server_err}" "${server_status}"
    RESULT_VARIABLE launch_result)
if(NOT launch_result EQUAL 0)
    message(FATAL_ERROR "Could not start local OpenAI-compatible loopback")
endif()
set(ready FALSE)
...
endforeach()
if(NOT ready)
    message(FATAL_ERROR "Local OpenAI-compatible loopback did not become ready")
endif()
...
endforeach()
if(NOT exited)
    message(FATAL_ERROR "Local OpenAI-compatible loopback did not exit")
endif()
...
if(NOT server_result STREQUAL "0")
    message(FATAL_ERROR "Local OpenAI-compatible loopback failed: ${server_result}")
endif()

message(STATUS
    "P1-LLM-02 passed configurable local OpenAI-compatible generation/embedding on rxvme/rxbvm")
