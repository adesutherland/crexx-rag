...
#define BUILD_DLL
#define PLUGIN_ID rx_sqlite_boundary
#include "crexxpa.h"
...
static int call_open(const char *path, mock_value *database) {
    ...
    sqlite_open_mode_boundary(3, arguments, &returned, &signal);
...
static int call_exec(mock_value *database, const char *sql) {
    ...
    sqlite_exec_boundary(2, arguments, &returned, &signal);
    return signal.integer ? -100 : (int)returned.integer;
}
...
static int call_busy_timeout(mock_value *database, int milliseconds) {
    ...
    arguments[0] = database;
    arguments[1] = &timeout;
    sqlite_busy_timeout_boundary(2, arguments, &returned, &signal);
    return signal.integer ? -100 : (int)returned.integer;
}
...
static int call_close(mock_value *database) {
    ...
    arguments[0] = database;
    sqlite_close_boundary(1, arguments, &returned, &signal);
    return signal.integer ? -100 : (int)returned.integer;
}
...
static int diagnostic_contains(const char *expected) {
    ...
    arguments[3] = &message;
    sqlite_error_boundary(4, arguments, &returned, &signal);
    return !signal.integer && returned.integer == SQL_BOUNDARY_SQLITE_ERROR &&
           code.integer == SQL_BOUNDARY_SQLITE_ERROR &&
...
static void *run_worker(void *opaque_arguments) {
    ...
    sqlite_boundary_session *session = sqlite_boundary_session_create();
    ...
    if (!session || sqlite_boundary_session_enter(
            session, RXPA_PROCEDURE_CAP_SESSION_AFFINE, &previous) != 0) {
        arguments->result = 1;
        sqlite_boundary_session_destroy(session);
#if defined(_WIN32)
        return 0;
#else
        return NULL;
    ...
    if (call_exec(&database, sql) != SQL_BOUNDARY_SQLITE_ERROR ||
    ...
    sqlite_boundary_session_leave(previous);
    sqlite_boundary_session_destroy(session);
...
static int verify_handle_session_isolation(const char *path) {
    sqlite_boundary_session *owner = sqlite_boundary_session_create();
    sqlite_boundary_session *other = sqlite_boundary_session_create();
    mock_value database = {0};
    void *previous = NULL;
    int result = 0;
    if (!owner || !other || sqlite_boundary_session_enter(
    ...
    sqlite_boundary_session_leave(previous);
    previous = NULL;
    if (sqlite_boundary_session_enter(
            other, RXPA_PROCEDURE_CAP_SESSION_AFFINE, &previous) != 0) {
        result = 3;
        goto restore_owner;
    }
    if (call_exec(&database, "SELECT 1") != SQL_BOUNDARY_INVALID_HANDLE) {
        result = 4;
    }
    sqlite_boundary_session_leave(previous);
    previous = NULL;
...
leave_owner:
    ...
    sqlite_boundary_session_destroy(other);
    sqlite_boundary_session_destroy(owner);
    return result;
}
...
int main(int argc, char **argv) {
    ...
    if (!manifest || manifest->abi_version != RXPA_PLUGIN_MANIFEST_ABI_V2 ||
        ...
        manifest->procedure_capabilities("sqlite_boundary.sqliteexec") !=
    ...
    puts("P1_SQL_THREAD_SESSIONS_OK sessions=4 rows=400 diagnostics_isolated=1 handle_isolation=1 fullmutex=1");
    return 0;
}
